...
基座——刻意保持 SQLite 自身的 `FULL` 而不静默降级。
...
④ sqlite 缺省即 `FULL`（逐提交 fsync），故它只有一个档位；`?sync=0` 可换 `NORMAL`。
⑤ `sync=1` 档位对比的是同一介质（真实 ext4）上的缺省档：**jsonl 慢 ~570×、bolt ~56×、
leveldb ~128×、badger ~109×**。这是"高速缺省"的全部意义所在——把逐提交持久化从
热路径上摘掉。反过来说：**需要崩溃不丢数据时，`sync=1` 的吞吐就是 ~377–1.3k ops/s
这个量级**，此时应优先用 `Batch` 摊薄提交，而不是逐条写。
...
- **缺省档把 fsync 从写热路径摘掉后，嵌入式基座的写吞吐提升 1–2 个数量级**：同一真实
  ...
  这是本版最重要的一组数字：**它们说明"要不要 `sync=1`"比"选哪个基座"影响更大**。
...
- **bolt 在缺省档仍有 18.8×**：它每写必开一个 bbolt 事务，该开销与介质无关，因此在
  tmpfs 上照样能被批掉（21.7×）。
- **sqlite 缺省即耐久，所以它缺省档的批写收益本来就高（54.4×）**——这正是"缺省即
  fsync"的基座该有的表现，也解释了为什么它的 Set 只有 ~559 ops/s。
