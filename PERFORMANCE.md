...
**五个本地基座的缺省档取向已统一**：都不逐提交 fsync，`?sync=1` 才要断电安全。
sqlite 的缺省由 `FULL` 改为 `NORMAL`（2024 版语义调整），因此**它的缺省吞吐比历史
记录高一个数量级**：同一真实 ext4 上 Set ~559 → **11.1k–14.7k**（约 23×）。需要
"掉电也不丢已确认写入"时必须显式 `?sync=1`。

**缺省档不等于"永不落盘"**，但各基座的兜底层次不同，不要混为一谈：

- **jsonl / bolt**：数据先落在**进程内存缓冲**里 → 进程崩溃就会丢。因此必须补有界
  兜底：bolt 按 `SyncInterval`（缺省 1s）周期落盘、jsonl 缓冲档按空闲 100ms 落盘，
  且两者 `Close` 都强制落盘一次（bbolt 自身的 `Close` **不做** fdatasync）。strace
  实证：静置 600ms、写入 1 次时，`sync_interval=100ms` 触发 9 次 fdatasync，调到 5s
  降到 4 次——落盘频率确由该参数控制。
- **sqlite（NORMAL）**：每次提交都已 `write()` 进 **OS 页缓存**，WAL 里有全部记录 →
  ...
  **按帧数**触发（`DEFAULT_WAL_AUTOCHECKPOINT=1000` 页），**没有任何时间驱动的
  checkpoint**：实测写 3000 条（越过阈值，主库 4KB→11.9MB）后静置 5s，主库与 WAL
  字节数完全不变。所以它**不需要**周期落盘兜底——缺省档的丢失边界就是"掉电丢最近
  若干提交"，要消除它只能 `?sync=1`。

旧参数 `nosync` 已全部废弃并 **报错**（不静默接受）：缺省即不 fsync，残留的
"写了 nosync 才高速"的认知会让人误判持久化等级。
...
**文件基座 @ ext4(WSL vhdx)，缺省档（高速：不逐条 fsync）**

| 基座 | Set | Get | Incr多key | Incr同key | QPush | MGet条目 | 批写条目 |
|---|---|---|---|---|---|---|---
| jsonl | ~215.5k | ~10.0M | ~190.1k | ~242.2k | ~268.9k | ~14.1M | ~498.7k |
| jsonl④ | ~330k | — | — | — | — | — | — |
| bolt | ~25.2k | ~591.1k | ~22.9k | ~26.9k | ~20.9k | ~2.6M | ~473.8k |
| leveldb | ~164.6k | ~1.0M | ~111.8k | ~122.4k | ~111.8k | ~1.0M | ~354.2k |
| badger | ~81.8k | ~274.4k | ~63.5k | ~34.0k | ~64.7k | ~608.2k | ~493.8k |
| sqlite | ~12.6k | ~62.9k | ~5.0k | ~5.8k | ~5.5k | ~450.9k | ~37.6k |

**文件基座 @ ext4(WSL vhdx)，`?sync=1`（逐提交 fsync）**

| 基座 | Set | Get | Incr多key | Incr同key | QPush | MGet条目 | 批写条目 |
|---|---|---|---|---|---|---|---
| jsonl | ~377 | ~10.0M | — | — | ~410 | ~14.1M | — |
| bolt | ~449 | ~591.1k | — | — | ~464 | ~2.6M | — |
| leveldb | ~1.3k | ~1.0M | — | — | ~1.3k | ~1.0M | — |
| badger | ~748 | ~272.0k | — | — | ~736 | ~608.2k | — |
| sqlite | ~440 | ~61.3k | — | — | — | ~453.0k | ~31.0k |

③ sqlite 的 tmpfs 数字不受缺省改动影响：tmpfs 上 fsync 是空操作，实测 NORMAL 12.9k
与 FULL 12.6k 基本一致，两档在内存盘上无法区分。**缺省改为 NORMAL 的收益只体现在
真实块设备上**（下一张表）。
④ `?buffered=1`（缓冲 + 空闲 100ms 落盘）：Set ~330k，约为缺省档的 1.5×。
⑤ `sync=1` 档位对比同一介质（真实 ext4）上的缺省档：**jsonl 慢 ~570×、bolt ~56×、
leveldb ~128×、badger ~109×、sqlite ~29×**。这是"高速缺省"的全部意义——把逐提交
持久化从热路径上摘掉。反过来说：**需要掉电也不丢数据时，`sync=1` 的吞吐只有
~377–1.3k ops/s**，此时应优先用 `Batch` 摊薄提交，而不是逐条写。
...
① jsonl 缺省只 flush 到 OS、不逐条 fsync（`?sync=1` 才是每次写 fsync）：跨基座比较前
需先对齐持久化等级（见 §1.1）。
② badger 缺省不 fsync（`?sync=1` 开启）；它旧版缺省是逐条 fsync，故历史数据中
"badger 慢"的结论对应 `sync=1` 档。它的读写会被提交窗口耦合，见 §2.3。

### 2.1 从表中读出的结论

- **读路径基本不受介质支配**：同一基座的 Get/MGet 在 tmpfs/ext4/9p 三档上几乎不变
  （如 leveldb Get ~0.93–1.0M、jsonl Get ~9.4–10.1M），因为读命中页缓存；而 Set/Incr/
  QPush 这类写路径跨介质差 2–3 个数量级。
- **缺省档把 fsync 从写热路径摘掉后，嵌入式基座的写吞吐提升 1–2 个数量级**：同一真实
  ext4 上，jsonl 377 → 215k、bolt 449 → 25.2k、leveldb 1.3k → 165k、badger 748 → 82k、
  sqlite 440 → 12.6k。这是本版最重要的一组数字：**"要不要 `sync=1`"比"选哪个基座"
  影响更大**。
- **sqlite 的缺省改动会改变它在表中的位置**：缺省从 FULL 改为 NORMAL 后，ext4 上
  Set 从 ~559 升到 ~12.6k（约 23×），从"最慢的嵌入式基座"变成**快于 bolt、
  慢于 badger** 的中档。历史引用的"sqlite ~559/批写 30.4k"对应的是今天的 `?sync=1`。
...
- **badger 是唯一"读会被写提交窗口耦合"的嵌入式基座**：纯读与其他嵌入式同级
  （~270k），但它缺省已不 fsync，故缺省下混合读不再被提交窗口拖垮；`sync=1` 时读保留率
  会掉到 0.2–0.4%（§2.3）。
- **jsonl 用缓冲档还能再快 1.5×**（215k → 330k），代价是写失败延迟上报与最多 32KiB
  尾部丢失，属于"先想清楚再开"的档位（§2.4）。

### 2.2 批写收益（批写条目吞吐 ÷ Set 吞吐）

| 基座 | tmpfs | ext4 缺省 | ext4 `sync=1` | drvfs(9p) | 服务端 |
|---|---|---|---|---|---|
| jsonl | 2.5× | 2.3× | — | 81.3× | — |
| bolt | 21.7× | 18.8× | ~86× | 80.4× | — |
| leveldb | 2.8× | 2.2× | ~71× | 74.4× | — |
| badger | 6.9× | 6.0× | ~39× | 83.7× | — |
| sqlite | 3.3× | 3.0× | ~70× | 68.8× | mysql 6.4× / pg 3.7× |
| redis | — | — | — | — | 39.0× |
| ssdb | — | — | — | — | 2.6× |

收益取决于单条写中可被摊薄的提交成本：

- **批写的收益与"单条写里有多少提交成本"成正比**：`sync=1` 下一次提交含一次 fsync，
  批写能把 N 次 fsync 摊成 1 次，故倍数高达 39–86×；缺省档已无 fsync，倍数回落到
  2.2–6.0×（剩下的可摊薄成本是事务/Batch 本身的固定开销）。
- **bolt 在缺省档仍有 18.8×**：它每写必开一个 bbolt 事务，该开销与介质无关，因此在
  tmpfs 上照样能被批掉（21.7×）。
- **sqlite 缺省改为 NORMAL 后，批写收益从 54.4× 回落到 3.0×**：原先的高倍数来自
  "缺省即逐提交 fsync"（单条写被 fsync 支配，批写一摊就出倍数）；去掉逐提交 fsync 后
  它与其它非耐久档基座回到同一量级。要恢复高倍数只能同时开 `?sync=1`（此时 ~70×）。
- **9p 档收益普遍更高**（68–84×）：连 flush 都要过协议往返，单条写被往返成本支配。
- SQL 系收益偏低（mysql 6.4×、pg 3.7×）是因为批内仍逐条语句执行，只是共享一次
  提交；Redis 39× 来自 MULTI/EXEC 一次往返 + 服务端合并。
...
| jsonl | ext4（缺省） | ~474k | ~105k | 5% | 49% |
| jsonl | 9p | ~4.45M | ~1.5k | 46% | 106% |
| bolt | tmpfs | ~166k | ~12.9k | 29% | 58% |
| bolt | ext4（缺省） | ~170k | ~13.5k | 27% | 53% |
| bolt | ext4（`sync=1`） | ~500k | ~491 | 85% | 115% |
| bolt | 9p | ~319k | ~119 | 54% | 134% |
| leveldb | tmpfs | ~109k | ~58.8k | 11% | 43% |
| leveldb | ext4（缺省） | ~98.7k | ~52.4k | 11% | 32% |
| leveldb | ext4（`sync=1`） | ~736k | ~827 | 74% | 64% |
| leveldb | 9p | ~658k | ~658 | 60% | 66% |
| badger | tmpfs | ~74.6k | ~40.3k | 27% | 60% |
| badger | ext4（缺省） | ~76.4k | ~49.7k | 28% | 61% |
| badger | ext4（`sync=1`） | ~1.0k | ~689 | 0.4% | 91% |
| badger | 9p | ~497 | ~314 | 0.2% | 98% |
| sqlite | tmpfs | ~37k | ~6.7k | 61% | 56% |
| sqlite | ext4（缺省） | ~40.1k | ~5.8k | 64% | 46% |
| sqlite | ext4（`sync=1`） | ~74k | ~409 | 121% | 93% |
...
- **mem / jsonl 在高写频率下读掉到纯读的 ~5–9%**（tmpfs、ext4）。jsonl 的读路径不取
  ...
  后续读者排队。`jsonl` 的纯读吞吐（Get ~9.4–10.1M）正说明其自身读路径没有额外代价。
...
- **badger 的混合读由"写提交窗口"支配，而不是写频率**：`sync=1` 时一次提交在 ext4/9p
  ...
  91–98%，说明写本身没被拖慢）。**缺省档（不 fsync）已消除这一现象**：同一负载在 ext4
  上读回到 ~76.4k、保留率 ~28%，与其他嵌入式同级。历史数据中"badger 混合读仅 0.2%"
  对应的是旧缺省（= 逐条 fsync），也就是今天的 `?sync=1` 档。
- **缺省档把写频率推高后，读保留率并不会更差**：bolt/leveldb/badger 的 ext4 缺省档写
  频率比 `sync=1` 高 1–2 个数量级，读保留率仍在 11–28%（`sync=1` 时反而更高，因为写者
  几乎不进锁）。这说明这些基座的读阻塞主要来自"写者占据同步原语的**时长**"，而非次数。
- 读保留率 >100%（sqlite ext4/9p）是该档纯读基准自身的调度与页缓存波动，属噪声量级。

### 2.4 jsonl 的缓冲档（`?buffered=1`）

jsonl 缺省逐操作 flush（不 fsync），保证**写失败立刻报错**；缓冲档把 flush 也省掉，
换取更高写吞吐：

| 档位 | ext4 Set | 落盘时机 | 写失败何时可见 |
|---|---|---|---|
| 缺省 | ~215.5k | 每次操作交给 OS | 当次操作立即返回错误 |
| `?buffered=1` | ~330k（1.5×） | 缓冲满 32KiB，或**空闲 100ms** | 最多延迟到下次落盘/Close |
| `?sync=1` | ~377 | 每次操作 flush+fsync | 当次操作立即返回错误 |

**"空闲落盘"是缓冲档的兜底**：连续 100ms（`Config.FlushInterval` 可调）没有新写入就
主动 flush，因此**不存在"没有后续写入就永远不落盘"**。实现上复用一个 `time.Timer` 做
`Reset`（实测 67 ns/op，约为写操作的 2%；每次新建 `AfterFunc` 要 318 ns/op），且回调
只在真正空闲时触发，不与写路径争锁——**实测缓冲档比缺省档快 1.5×，守护机制零拖累**。
后台 flush 失败会记为粘性错误，由下一次写或 `Close` 上报，不会被静默吞掉。

代价（都写在该档的文档注释里）：进程崩溃最多丢"最近 100ms + 缓冲内 32KiB"的尾部记录；
写失败不再归因到出错的那次操作。
...
|---|---|---|---|---|---|---|---|---|---|
| Set | map 写 | 1 次 append+flush | 1 个事务（缺省无 fsync） | 1 次 `Write(batch)`（缺省无 fsync） | 1 个 `Update` 事务（缺省无 fsync） | 1 条 upsert（sqlite 缺省 FULL=1 fsync） | 1 条 upsert | 1 命令 | 1 往返 |
