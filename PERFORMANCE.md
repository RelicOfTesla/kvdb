...
| 基座 | Set | Get | Incr多key | Incr同key | QPush | MGet条目 | 批写条目 |
|---|---|---|---|---|---|---|---
| jsonl | ~353.6k | ~10.0M | ~190.1k | ~242.2k | ~639.3k | ~14.1M | ~454.7k |
...
真实块设备上**（下一张表）。
② `sync=1` 档位对比同一介质（真实 ext4）上的缺省档：**jsonl 慢 ~940×、bolt ~56×、
...
① 嵌入式基座缺省档都不逐提交 fsync（jsonl 缺省 flush 500ms/fsync 1s）：跨基座比较前
需先对齐持久化等级（见 §1.1）。
