// Command rpcdemo 把任意 kvdb 基座暴露成 RPC 服务端，配合 client 实现
// "本地变远程"的 c/s 隔离。
//
// 用法：
//
//	# 最简：jsonl 底座 + 挑战认证（推荐）
//	go run ./example/rpcdemo -addr :7788 -backend jsonl://./tmp/data.jsonl \
//	    -auth challenge -password s3cret
//
//	# 本地/内网裸奔（无认证、无 TLS）
//	go run ./example/rpcdemo -addr :7788 -backend mem://
//
//	# TLS + 挑战认证（生产组合）
//	go run ./example/rpcdemo -addr :7788 -backend sqlite://./tmp/data.db \
