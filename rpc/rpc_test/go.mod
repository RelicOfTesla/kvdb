module github.com/RelicOfTesla/kvdb/rpc/rpc_test

go 1.25.0

require (
	github.com/RelicOfTesla/kvdb v0.0.0
	github.com/RelicOfTesla/kvdb/badger v0.0.0-00010101000000-000000000000
	github.com/RelicOfTesla/kvdb/bolt v0.0.0-00010101000000-000000000000
	github.com/RelicOfTesla/kvdb/jsonl v0.0.0-00010101000000-000000000000
	github.com/RelicOfTesla/kvdb/leveldb v0.0.0-00010101000000-000000000000
	github.com/RelicOfTesla/kvdb/mem v0.0.0
	github.com/RelicOfTesla/kvdb/rpc v0.0.0
	github.com/RelicOfTesla/kvdb/sqlite v0.0.0-00010101000000-000000000000
)

require (
	github.com/RelicOfTesla/kvdb/sqlstore v0.0.0 // indirect
	github.com/cespare/xxhash/v2 v2.3.0 // indirect
	github.com/dgraph-io/badger/v4 v4.9.6 // indirect
	github.com/dgraph-io/ristretto/v2 v2.2.0 // indirect
	github.com/dustin/go-humanize v1.0.1 // indirect
	github.com/go-logr/logr v1.4.3 // indirect
	github.com/go-logr/stdr v1.2.2 // indirect
	github.com/golang/snappy v0.0.0-20180518054509-2e65f85255db // indirect
	github.com/google/flatbuffers v25.2.10+incompatible // indirect
	github.com/google/uuid v1.6.0 // indirect
	github.com/klauspost/compress v1.18.0 // indirect
	github.com/mattn/go-isatty v0.0.24 // indirect
	github.com/ncruces/go-strftime v1.0.0 // indirect
	github.com/remyoudompheng/bigfft v0.0.0-20230129092748-24d4a6f8daec // indirect
	github.com/syndtr/goleveldb v1.0.0 // indirect
	go.etcd.io/bbolt v1.5.0 // indirect
	go.opentelemetry.io/auto/sdk v1.2.1 // indirect
	go.opentelemetry.io/otel v1.41.0 // indirect
	go.opentelemetry.io/otel/metric v1.41.0 // indirect
	go.opentelemetry.io/otel/trace v1.41.0 // indirect
	golang.org/x/sys v0.47.0 // indirect
	google.golang.org/protobuf v1.36.7 // indirect
	modernc.org/libc v1.75.6 // indirect
	modernc.org/mathutil v1.7.1 // indirect
	modernc.org/memory v1.12.1 // indirect
	modernc.org/sqlite v1.58.0 // indirect
)

replace (
	github.com/RelicOfTesla/kvdb => ../../
	github.com/RelicOfTesla/kvdb/badger => ../../badger
	github.com/RelicOfTesla/kvdb/bolt => ../../bolt
	github.com/RelicOfTesla/kvdb/jsonl => ../../jsonl
	github.com/RelicOfTesla/kvdb/leveldb => ../../leveldb
	github.com/RelicOfTesla/kvdb/mem => ../../mem
	github.com/RelicOfTesla/kvdb/rpc => ../
	github.com/RelicOfTesla/kvdb/sqlite => ../../sqlite
	github.com/RelicOfTesla/kvdb/sqlstore => ../../sqlstore
)
